...
class AEM{
	...
	public boolean addEntityToModel(Entity e) throws ExecException{
					...
					if ( e.aemLastType == this.TYPE_SRAL && Math.abs(lastEntity.overlap(e))>= this.READING_TIME)
						...
						for ( int i = activities.size()-1; i >= 0; i--){ // reversed order
							...
							if ( act.contains(lastEntity)){
								...
								e.aemPredType = e.aemLastType;
								e.hasFakeReferrer = true;
								linked2activity = true;
								act.addEntity(e, lastEntity);
								break;
							}
						}
					}
		...
		if ( createNew || !linked2activity ){
			...
			String ref = e.referrer;
			if ( ref != null ){
				Entity dummyEntity = new Entity(e.start, null, e.referrer, null, null, null, null);
